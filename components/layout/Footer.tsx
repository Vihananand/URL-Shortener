      ...
      <div className="max-w-5xl mx-auto px-5 sm:px-8 py-14">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 mb-12">
            ...
            <div className="flex items-center gap-2 mt-5">
              {socials.map((s) => (
                <a key={s.label} href="#" aria-label={s.label}
                  className="w-7 h-7 flex items-center justify-center rounded-lg border border-border text-white/30 hover:text-white/70 hover:border-border2 transition-all duration-200">
          ...
          {Object.entries(footerLinks).map(([cat, links]) => (
            <div key={cat}>
              <h4 className="text-[10px] font-semibold uppercase tracking-widest text-white/30 mb-4">{cat}</h4>
              <ul className="space-y-2.5">
                {links.map((l) => (
                  <li key={l.label}>
                    <Link href={l.href} className="text-xs text-white/40 hover:text-white/80 transition-colors duration-200">
                      {l.label}
                    </Link>
                  </li>
                ))}
