...
// Rate limiter: 10 password attempts per minute per IP
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Verify active status, expiration, and clicks again just in case
    ...
    if (!url.password_hash) {
      // If it somehow doesn't have a password, just return the URL
